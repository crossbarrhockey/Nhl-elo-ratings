...
def compute(force_refresh=False):
...
st.caption(
    ...
    "Shown ratings are 2026-27 preseason values after 30 percent regression to 1505."
)
...
try:
    ...
    progress.empty()
    st.error("Could not load NHL data: " + str(e))
...
if view == "2026-27 Preseason Rankings":
    ...
    st.caption(
        ...
        + str(len(games))
        + " | "
        + str(pd.to_datetime(games["date"]).min().date())
        + " to "
        + str(pd.to_datetime(games["date"]).max().date())
    )
    ...
    if home and away and home != away:
        ...
        st.metric(TEAM_NAMES[home] + " win probability", "{:.1f}%".format(p * 100))
        st.metric(TEAM_NAMES[away] + " win probability", "{:.1f}%".format((1 - p) * 100))
        st.write(
            "Home Elo {:.1f}  |  Away Elo {:.1f}".format(preseason[home], preseason[away])
        )
    ...
    st.markdown(
        ...
        - Playoff EloDiff x 1.25
        - 30 percent reversion to 1505 after each season
